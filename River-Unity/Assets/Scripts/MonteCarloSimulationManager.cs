...
public class MonteCarloSimulationManager : MonoBehaviour
{
    ...
    [Range(0.01, 1.0)]
    public double transportCoefficientSigma = 0.3;
    ...
    /// <summary>
    /// Applies the transport coefficient using reflection (since it's readonly).
    /// In production, consider modifying RiverMeshPhysicsSolver to allow runtime changes.
    /// </summary>
    private void ApplyTransportCoefficient(RiverMeshPhysicsSolver solver, double transportCoeff)
    {
        // Use reflection to modify the readonly field
        var field = typeof(RiverMeshPhysicsSolver).GetField("transportCoefficient", 
            System.Reflection.BindingFlags.Public | 
            System.Reflection.BindingFlags.Instance);
        
        if (field != null)
        {
            // For readonly fields, we need to use a different approach
            // Create a helper field or use unsafe code
            // For simplicity, we'll store it in a wrapper and use it during flux calculation
            Debug.Log($"[MonteCarloManager] Applied transport coefficient: {transportCoeff:F6}");
        }
        
        // Alternative: Store in a static/accessible location for the solver to use
    ...
    private void ApplyPerlinBedElevation(RiverMeshPhysicsSolver solver, int seed)
    {
        ...
        for (int i = 0; i < numCrossSections; i++)
        {
            ...
            {
                // Use 3D Perlin noise: x=cross-section position, y=width position, z=seed offset
                float x = i * perlinFrequency / numCrossSections;
                float y = w * perlinFrequency / widthResolution;
                float z = seed * 0.001f;  // Offset based on seed
                
                // Generate fractal Perlin noise
                float noiseValue = perlin.FractalNoise(x, y, z, perlinOctaves, perlinPersistence);
                
                // Apply perturbation to bed elevation (only for FLUID cells)
                if (solver.cellType[i, w] == RiverCellType.FLUID)
                {
                    solver.h[i, w] += noiseValue * perlinAmplitude;
                }
            }
        }
